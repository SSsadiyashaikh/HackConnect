...
const HackathonDetails = () => {
  ...
  useEffect(() => {
    if (window.location.hash === '#participants') {
      const el = document.getElementById('participants-section');
      if (el) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {user?.role === 'organizer' && (hackathon.organizer?._id === user?.id || hackathon.organizer === user?.id) && (
        <div
          ...
          {(!hackathon.participants || hackathon.participants.length === 0) ? (
            ...
            <ul className="divide-y divide-gray-200">
              {hackathon.participants.map((participant) => (
                <li key={participant._id || participant} className="py-3">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      {participant.profile?.skills && participant.profile.skills.length > 0 && (
                        <div className="mt-1 flex flex-wrap gap-2">
                          {participant.profile.skills.map((skill, idx) => (
                            <span
                              key={idx}
