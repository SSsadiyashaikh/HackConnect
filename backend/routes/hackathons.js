...
router.post('/', authenticate, isOrganizer, [
  ...
  try {
    ...
    // Create notifications for interested students about the new hackathon
    try {
      // Try to target students whose interests match the hackathon domain
      const interestQuery = { role: 'student' };
      if (hackathon.domain && hackathon.domain !== 'Other') {
        interestQuery['profile.interests'] = { $in: [hackathon.domain] };
      }

      let students = await User.find(interestQuery);

      // Fallback: if no interested students found, notify all students
      ...
      if (students.length > 0) {
        const notifications = students.map((student) => ({
          ...
          message: `${hackathon.title} has just been announced. Check it out!`,
      ...
      console.error('Error creating hackathon notifications:', notifyError);
    }
...
router.post('/:id/register', authenticate, async (req, res) => {
  try {
    ...
    const registrationNotification = await Notification.create({
      user: req.user._id,
      type: 'hackathon',
    ...
    // Emit real-time notification to the participant
    const io = req.app.get('io');
    if (io) {
      io.to(`user-${req.user._id.toString()}`).emit('notification', registrationNotification);
    }

    // Notify organizer about new participant registration
    try {
      ...
      const organizerNotification = await Notification.create({
        ...
        message: `${user.name} has registered for your hackathon ${hackathon.title}.`,
        relatedId: hackathon._id,
        relatedModel: 'Hackathon'
      });

      if (io) {
        io.to(`user-${hackathon.organizer.toString()}`).emit('notification', organizerNotification);
      }
    } catch (organizerError) {
      console.error('Error creating organizer registration notification:', organizerError);
    }
    ...
    try {
      ...
      if (userSkills.length > 0) {
        ...
        const teamNotifications = await Promise.all(
        ...
        // Emit real-time notifications to team leaders
        const io = req.app.get('io');
        if (io) {
